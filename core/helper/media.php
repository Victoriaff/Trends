...
class media {
	...
	public static function img( $args = array() ) {
		...
		$default_args = array(
			'url'    => \trends\helper\utils::sanitize_uri( \get_the_post_thumbnail_url( \get_the_ID(), 'full' ) ),
			'width'  => 800,
			'height' => 400,
			'crop'   => true,
			'hdmi'   => true,
		...
		// SVG
		$is_svg = \trends\helper\utils::is_attachment_svg( $args['attachment_id'],  $args['url'] );
				
		$src = \aq_resize( \trends\helper\utils::sanitize_uri( $args['url'] ), \absint( $args['width'] ), \absint( $args['height'] ), (bool) $args['crop'] );
		...
		if ( ! \Trends()->controller->lazy_load->skip() ) {
			return \Trends()->controller->lazy_load->add_image_placeholders( $_img );
		}
